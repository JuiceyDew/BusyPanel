...
The panel is reachable from the LAN and holds the books, so binding it to 0.0.0.0
without a gate means anyone on the network can read invoices or delete them. This
is a small self-contained login: one password, a signed session cookie, no
external proxy, no user table.

The password comes from, in precedence order:

  * ``AUTH_PASSWORD`` in the real environment (what the NixOS ``environmentFile``
    or systemd credential wrapper sets), or
  * ``auth_password`` in ``settings.json``, set with ``busypanel passwd`` or on
...
state directory (mode 0600), so sessions survive a restart. Stdlib only -- no new
dependency.
"""
